...
const format = (t: any) => (t ? `${pad(t.hour)}:${pad(t.minute)}` : "");
...
export function TimePicker({
  ...
  const hourRef = React.useRef<any>(null);
  const minuteRef = React.useRef<any>(null);
  ...
  React.useEffect(() => {
    if (!open) return;
    const sel = (col: any) => col && col.querySelector(".is-selected");
  ...
  function pickHour(hr: any) {
    setValue({ hour: hr, minute: time ? time.minute : 0 });
  }
  function pickMinute(mn: any) {
          ...
          h("div", { className: "ds-timepicker__col", ref: hourRef, role: "listbox", "aria-label": "Hour" },
            hours.map((hr) =>
              h("button", {
                ...
                onMouseDown: (e: any) => { e.preventDefault(); pickHour(hr); },
              }, pad(hr))
            )
          ),
          h("div", { className: "ds-timepicker__col", ref: minuteRef, role: "listbox", "aria-label": "Minute" },
            minutes.map((mn) =>
              h("button", {
                ...
                onMouseDown: (e: any) => { e.preventDefault(); pickMinute(mn); },
              }, pad(mn))
            )
          )
        )
