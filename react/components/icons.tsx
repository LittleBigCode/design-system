...
/** Lucide-compatible icon geometry, keyed by name (inner SVG markup, no wrapper). */

export type IconName = keyof typeof icons;
/* ---- Icon set ------------------------------------------------------------
   Lucide-compatible icon geometry. Each value is the *inner* SVG markup only
   (no <svg> wrapper); the wrapper — viewBox "0 0 24 24", fill="none",
   stroke="currentColor", stroke-width 2, round caps/joins — is supplied by the
   <Icon> React component, the <ds-icon> web component, or written by hand in
   static markup. Because the conventions match Lucide exactly, any Lucide icon
   can be dropped in here and will render identically.

   Geometry is adapted from Lucide (https://lucide.dev), ISC-licensed — a
   permissive license that allows reuse. Drawn simply so each reads at 16–24px. */
export const icons = {
  search:
